import { SOCKET_EVENTS, DIRECTIONS, PLAYER_COLORS, GAME_CONFIG, POWERUP_TYPES } from '../../shared/constants.js';
...
export class GameLoop {
  ...
  renderPlayer(container, player, id) {
    ...
    if (!element) {
      ...
      if (spriteLoader.hasSprite(spriteKey)) {
        element.style.backgroundImage = `url(${spriteLoader.getSprite(spriteKey)})`;
      ...
      console.log(`Created player ${player.nickname} at (${player.x}, ${player.y})`);
    }
  ...
  renderBomb(container, bomb, id) {
    ...
    if (!element) {
      ...
      console.log(`Creating bomb element at grid (${bomb.x}, ${bomb.y}), pixel (${bomb.x * GAME_CONFIG.CELL_SIZE}, ${bomb.y * GAME_CONFIG.CELL_SIZE})`);
      
      // Use sprite if available
      if (spriteLoader.hasSprite('bomb')) {
        element.style.backgroundImage = `url(${spriteLoader.getSprite('bomb')})`;
        element.style.backgroundSize = 'cover';
  ...
  renderExplosion(container, cell, id) {
    ...
    if (!element) {
      ...
      if (spriteLoader.hasSprite('explosion')) {
        element.style.backgroundImage = `url(${spriteLoader.getSprite('explosion')})`;
        element.style.backgroundSize = 'cover';
      }
  ...
  renderPowerup(container, powerup, id) {
    ...
    if (!element) {
      ...
      console.log(`Creating powerup ${powerup.type} at grid (${powerup.x}, ${powerup.y})`);
